...
group_by(series, Série=series_name) %>% 
  summarize(Mediana=median(UserRating),Média=mean(UserRating),Máximo=max(UserRating),Mínimo=min(UserRating)) %>%
...
ggplot(sherlock, aes(series_name, UserRating, series_ep)) + theme_bw() +
  geom_line(data=sherlock, aes(x=series_ep, y=UserRating), color="blue") +
