...
```{r include=FALSE}
summarise.series.median <- function(dados) {
  dados %>%
    group_by(series_name, season) %>%
    summarise(mediana=median(UserRating))
}

summarise.season.median <- function(series_median) {
  series_median %>%
    group_by(season) %>%
    summarise(mediana=median(mediana))
}
series_median <- summarise.series.median(dados)
seasons_median <- summarise.season.median(series_median)
```
...
```
```{r include=FALSE}
mtf_series_median <- summarise.series.median(moreThanFour)
mtf_seasons_median <- summarise.season.median(mtf_series_median)
```
...
```{r moreThanFiveSeriesTimeSeries}
mtf_seasons_median %>%
  ggplot(aes(x=season, y=mediana)) + theme_bw() +
  geom_line() +
